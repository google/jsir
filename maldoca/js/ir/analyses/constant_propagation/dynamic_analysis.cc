...
#include "maldoca/js/ir/analyses/constant_propagation/symbol_mutation_info.h"
...
void JsirDynamicConstantPropagationAnalysis::VisitOp(
  ...
  // No prelude: same behavior as ordinary constant propagation.
  if (dynamic_prelude_ == nullptr) {
...
JsirDynamicConstantPropagationAnalysis::EvalIdentifier(
  ...
  if (dynamic_prelude_ != nullptr &&
      dynamic_prelude_->GetFunction(scopes_, symbol_id).has_value()) {
